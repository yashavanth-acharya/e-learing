...
class Singup(TemplateView):
    ...
    def post(self,request):
        ...
        if Userdetails.objects.filter(username=username.lower()):
        ...
        try:
            Userdetails(
                ...
                # state= state,
                pincode=pincode,
                password=str(gethashpassword(password))
            ).save()
    
            return JsonResponse({'data':"Account created successfully","error":0})
